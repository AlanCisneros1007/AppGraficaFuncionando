...
/*
		...
		for (int i = 0; i < 50; i++) {
			if (registros[i].clavemaquina != 0) {
				String^ xd = gcnew String(registros[i].correoelectronico);
				MessageBox::Show(""+registros[i].clavemaquina+xd);
			}
		}
	}
...
void Baja(int clave) {
	...
	for (int i = 0; i < 50; i++) {
		...
		if (registros[i].clavemaquina == clave && clave != 0) {
			String^ nombremaq = gcnew String(registros[i].nombremaquina);
			String^ nombprov = gcnew String(registros[i].nombreproveedor);
			String^ ncelular = gcnew String(registros[i].numerocelular);
			String^ correoe = gcnew String(registros[i].correoelectronico);

			MessageBox::Show("----------------------------------------------------------\n"+ registros[i].clavemaquina+"\n"+ "Registro dado de baja con exito. ");
	...
	if (encontrado == false) {
		MessageBox::Show("Lo sentimos, el ID ingresado no existe en la base de datos. ");
	}
}